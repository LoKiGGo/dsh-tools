...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * live inside 功能开关 and get no tab of their own; wechat.openclaw is a
		 * normal panel feature and gets its 「微信接入」tab through this rule).
		...
		function UsagePanel(props) {
			...
			const priceMode = usageCfg.priceMode === "peak" ? "peak" : "offPeak";
			...
			const togglePriceMode = () => {
				const next = priceMode === "peak" ? "offPeak" : "peak";
				const c = featureConfigOf(cfg, "ui.usage");
				setFeatureConfig("ui.usage", Object.assign({}, c, { priceMode: next })).catch((error) => {
				...
				(empty || chartLoading) ? el("p", { className: "dt-ug-empty" }, chartLoading ? "正在加载用量数据…" : "所选范围内暂无用量数据。") : el("div", { className: "dt-ug-section" },
					el("div", { className: "dt-ug-kpis" },
						...
						el("div", { className: "dt-ug-kpi" },
							...
							el("div", { className: "dt-ug-kpiSubRow" },
								el("span", { className: "dt-ug-kpiSub" }, "估算"),
								el("button", {
									type: "button",
									className: "dt-ug-priceModeBtn" + (priceMode === "peak" ? " peak" : " valley"),
									title: priceMode === "peak" ? "当前按高峰价格计算，点击切换为谷价" : "当前按谷（空闲）价格计算，点击切换为高峰价",
									"aria-label": priceMode === "peak" ? "切换到谷价计算" : "切换到峰价计算",
					...
					el("div", { className: "dt-ug-priceConfig" },
						...
						priceOpen ? el("div", { className: "dt-ug-priceConfig" },
							...
							el("div", { className: "dt-ug-priceHint" }, "价格单位：元 / 百万 tokens。默认按谷（空闲）价格计算；可点击费用卡片上的峰/谷按钮切换，或在 JSON 中把 priceMode 改为 peak。"),
					...
					el("p", { className: "dt-ug-note" }, "数据仅供参考，与真实数据有差异。"),
				),
			);
		}
