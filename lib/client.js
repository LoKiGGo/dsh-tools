	...
	factory: (require) => {
		...
		function DeleteChatPanel(props) {
			...
			const load = () => {
				...
				apiDelete("list").then((sessions) => {
					setData({ loading: false, error: null, sessions: Array.isArray(sessions) ? sessions : [] });
				}, (err) => {
					setData({ loading: false, error: "加载失败: " + String((err && err.message) || err), sessions: [] });
				});
			};
			...
			const renderArchivedTab = () => {
				...
				if (archived.length === 0) return el("div", { className: "dsh-delchat-muted" }, "没有归档的会话。");
				return el("div", null,
		...
		function PluginTogglePanel() {
			...
			const toggle = (p) => {
				...
				apiToggle("set", { name: p.name, enabled: !p.enabled }).then((value) => {
					setBusy(null);
					setChanged(true);
					setNotice((value.enabled ? "已启用 " : "已停用 ") + p.name + "。重启 dsh web 后生效（可在本页顶部一键重启）。");
					load();
				}, (err) => {
					setBusy(null);
			...
			const restart = () => {
				...
				if (bridge !== undefined && typeof bridge.restartService === "function") {
					if (window.confirm("重启会中断当前正在运行的会话（历史记录保留）。确定现在重启服务吗？")) {
						bridge.restartService().catch(() => {});
					}
				}
			};

			const bridgeAvailable = typeof window !== "undefined" && window.dshDesktop !== undefined && typeof window.dshDesktop.restartService === "function";
			...
			return el("div", { className: "pt" },
				...
				el("p", { className: "pt-hint" }, "开关写入 profile 激活层（cordis.patch.yml / bundles），不会卸载插件；改动在重启 dsh web 后生效。"),
				...
				changed ? el("div", { className: "pt-restart", role: "status" },
					el("span", null, "激活层已修改，重启服务后生效。"),
					bridgeAvailable ? el("button", { className: "pt-btn primary", onClick: restart }, "立即重启服务") : null,
				) : null,
		...
		function UpdateCheckPanel() {
			const [phase, setPhase] = React.useState("checking"); // checking | done
			const [error, setError] = React.useState(null);
			const [plugins, setPlugins] = React.useState([]);
			...
			const load = () => {
				...
				setError(null);
				setNotice(null);
				apiUpdate("check").then((res) => {
					setPlugins(Array.isArray(res.plugins) ? res.plugins : []);
					setPhase("done");
				}, (err) => {
					setError("检查失败：" + String((err && err.message) || err));
					setPhase("done");
				});
			};
			...
			const updateRow = (name) => {
				...
				apiUpdate("update", { packageName: name }).then((res) => {
					...
					setRows(next);
					setBusy(null);
					setNotice("已更新 " + name + " 至 " + String(res.version || "最新") + "。重启 dsh web 后生效（可用本页顶部的一键重启）。");
				}, (err) => {
			...
			const statusText = () => {
				...
				return "共 " + plugins.length + " 个已安装插件，" + outdated.length + " 个可用更新。";
			};
			...
			return el("div", { className: "dsh-upd" },
				...
				phase === "done" && error === null
					? el("div", { className: "dsh-upd-muted" }, "提示：插件更新写入 profile 后，需重启 dsh web 才会加载新版本（可用本页顶部的一键重启）。")
					: null,
			);
		}
