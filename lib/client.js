...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const STYLES = [
			...
			{
				// 提问面板折叠（融合自 dsh-question-panel-collapse，MIT © Townrain）。
				id: "dsh-tools-question-collapse-style",
				css: [
					'.dsh-question-panel-collapse-on [data-question-key] [data-question-scroll] { display: none !important; }',
					'.dsh-question-panel-collapse-on [data-question-key] footer { display: none !important; }',
					'.dsh-question-panel-collapse-on [data-plan-review-key] [data-plan-review-scroll] { display: none !important; }',
					'.dsh-question-panel-collapse-on [data-plan-review-key] footer { display: none !important; }',
					'.dsh-question-panel-collapse-on [data-question-key] header { padding: 4px 10px 9px 14px !important; align-items: center; gap: 10px; }',
					'.dsh-question-panel-collapse-on [data-question-key] header > div:first-child { max-width: 55%; overflow: hidden; }',
					'.dsh-question-panel-collapse-on [data-question-key] header h2 { font-size: 13px !important; line-height: 20px !important; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }',
					'.dsh-question-panel-collapse-on [data-question-key] .dshqc-inpanel-btn { transform: translateY(2px); }',
					'.dshqc-in-panel { margin-left: auto; display: inline-flex; align-items: center; }',
					'.dshqc-inpanel-btn { display: inline-grid; place-items: center; width: 22px; height: 22px; padding: 0; border: none; border-radius: 999px; background: transparent; color: var(--dsw-alias-label-tertiary, inherit); cursor: pointer; opacity: .8; transition: opacity .12s ease, background-color .12s ease; }',
					'.dshqc-inpanel-btn:hover { opacity: 1; background: var(--dsw-alias-interactive-bg-hover, rgba(127,127,137,.12)); color: var(--dsw-alias-label-primary, inherit); }',
					'.dshqc-inpanel-btn:active { opacity: 1; background: var(--dsw-alias-interactive-bg-active, rgba(127,127,137,.2)); }',
					'.dshqc-chev { display: inline-block; width: 8px; height: 8px; border-right: 1.5px solid currentColor; border-bottom: 1.5px solid currentColor; transform: rotate(45deg); }',
					'.dshqc-chev.up { transform: rotate(-135deg); }',
					'.dshqc-fab { position: fixed; z-index: 1000; display: inline-grid; place-items: center; width: 26px; height: 26px; border: 1px solid rgba(255,255,255,.28); border-radius: 999px; background: rgba(30,34,40,.94); color: #f0f0f2; box-shadow: 0 2px 10px rgba(0,0,0,.35); cursor: pointer; transition: background-color .12s ease, transform .12s ease; pointer-events: auto; }',
					'.dshqc-fab:hover { background: rgba(45,50,58,.97); transform: translateY(-1px); }',
				].join("\n"),
			},
			{
			...
			{
				// 外观定制 token 覆盖（融合自 dsh-client-ui-custom，MIT © Yoli-mi）。
				// data-dsu-active 门控：未启用/中性配置时对原生界面零改动。
				id: "dsh-tools-ui-appearance-css",
				css: [
					"html[data-dsu-active='1'] body {",
					"  background-image: var(--dsu-gradient, none), var(--dsu-tone, none), var(--dsu-wallpaper, none);",
					...
					"  background-image: linear-gradient(180deg, var(--dsu-scrim, rgb(15 17 21 / .22)), var(--dsu-scrim, rgb(15 17 21 / .22))), var(--dsu-gradient, none), var(--dsu-tone, none), var(--dsu-wallpaper, none);",
					...
					"  backdrop-filter: blur(var(--dsu-blur, 0px)) saturate(var(--dsu-saturate, 1.25));",
					"  -webkit-backdrop-filter: blur(var(--dsu-blur, 0px)) saturate(var(--dsu-saturate, 1.25));",
					"  box-shadow: inset 0 0 40px rgb(15 17 21 / calc(var(--dsu-vignette, 0) * 0.10));",
					"}",
					"html[data-dsu-active='1'] [data-chat-anchor-key],",
					"html[data-dsu-active='1'] input:not([type='checkbox']):not([type='radio']):not([type='range']):not([type='color']),",
					"html[data-dsu-active='1'] textarea,",
					"html[data-dsu-active='1'] [role='dialog'],",
					"html[data-dsu-active='1'] [role='menu'] { border-radius: var(--dsu-radius); }",
					"html[data-dsu-active='1'] [role='dialog'],",
					"html[data-dsu-active='1'] [role='menu'] { box-shadow: var(--dsu-shadow); }",
					"html[data-dsu-active='1'] input:focus-visible,",
					"html[data-dsu-active='1'] textarea:focus-visible,",
					"html[data-dsu-active='1'] button:focus-visible,",
					"html[data-dsu-active='1'] [role='button']:focus-visible { box-shadow: 0 0 0 2px color-mix(in srgb, var(--dsu-accent, #4176e6) calc(var(--dsu-focus-glow, 0) * 45%), transparent); }",
				].join("\n"),
			},
			{
				// 外观设置面板表单样式。
				id: "dsh-tools-ui-appearance-panel-style",
				css: [
					'.dt-ap-field { display: flex; flex-direction: column; gap: 6px; }',
					'.dt-ap-field-head { display: flex; align-items: center; justify-content: space-between; gap: 8px; }',
					'.dt-ap-label { font-size: 13px; line-height: 20px; color: var(--dsw-alias-label-primary); }',
					'.dt-ap-value { font-size: 12px; color: var(--dsw-alias-label-tertiary); }',
					'.dt-ap-input, .dt-ap-select, .dt-ap-textarea { border: 1px solid var(--dsw-alias-border-l2); background: var(--dsw-alias-bg-layer-3); color: var(--dsw-alias-label-primary); font: inherit; font-size: 12px; border-radius: 6px; padding: 5px 8px; }',
					'.dt-ap-input:focus, .dt-ap-select:focus, .dt-ap-textarea:focus { outline: 2px solid var(--dsw-alias-state-business-primary); outline-offset: 1px; }',
					'.dt-ap-textarea { resize: vertical; font-family: ui-monospace, Consolas, monospace; }',
					'.dt-ap-presets { display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 8px; }',
					'.dt-ap-preset { display: flex; flex-direction: column; gap: 3px; text-align: left; border: 1px solid var(--dsw-alias-border-l2); border-radius: 8px; padding: 8px 10px; background: var(--dsw-alias-bg-layer-3); color: var(--dsw-alias-label-primary); font: inherit; cursor: pointer; }',
					'.dt-ap-preset.active { border-color: var(--dsw-alias-state-business-primary); outline: 1px solid var(--dsw-alias-state-business-primary); }',
					'.dt-ap-preset-name { font-size: 13px; font-weight: 600; }',
					'.dt-ap-preset-desc { font-size: 11px; color: var(--dsw-alias-label-tertiary); }',
					'.dt-ap-toggle { display: inline-flex; align-items: center; gap: 8px; font-size: 13px; color: var(--dsw-alias-label-primary); cursor: pointer; }',
					'.dt-ap-accent-row { display: inline-flex; align-items: center; gap: 8px; }',
					'.dt-ap-color { width: 34px; height: 26px; padding: 0; border: 1px solid var(--dsw-alias-border-l2); border-radius: 6px; background: transparent; cursor: pointer; }',
					'.dt-ap-swatches { display: flex; gap: 6px; flex-wrap: wrap; }',
					'.dt-ap-swatch { width: 22px; height: 22px; border-radius: 50%; border: 1px solid var(--dsw-alias-border-l2); cursor: pointer; padding: 0; }',
					'.dt-ap-swatch.active { outline: 2px solid var(--dsw-alias-state-business-primary); outline-offset: 1px; }',
				].join("\n"),
			},
		];
		...
		function SettingsPage(props) {
			...
			const mergedPanels = {
				...
				"ui.appearance": { heading: "外观", component: AppearancePanel },
			};

			const renderTabBody = () => {
				if (activeTab === "manage") {
					// Always-on cards first, then the notify permission card
					// (only while 任务完成提示 is enabled), then the toggle list.
					return [
						el("div", { key: "alwayson-restart" }, renderRestartCard()),
						featureEnabled(cfg, "notify.task-done")
				...
				if (activeTab === "ui-enhance") {
					return React.createElement(EnhancePanel, { close });
				}
			...
			return el("div", { className: "dt-page" },
				...
				el("p", { className: "dt-hint" }, "「功能开关」页签顶部为一键重启与桌面通知授权（任务完成提示开启时显示），下方为可选功能的开关。页签超出宽度时按住左右拖动查看。"),
				el("div", {
		 ...
		 * live inside 功能开关 and get no tab of their own), plus the merged
		 * 「界面增强」tab while ui.history or ui.markdown is enabled. Exposed
		 * via __dshToolsTest so the Node smoke test can assert tab derivation
		 * without a DOM.
		 */
		function tabModel(cfg) {
			...
			if (cfg !== null && cfg !== undefined) {
				...
				const anyEnhance = (cfg.features || []).some((f) => (f.key === "ui.history" || f.key === "ui.markdown") && f.enabled === true);
				if (anyEnhance) tabs.push({ key: "ui-enhance", label: "界面增强" });
			}
			return tabs;
		}
		...
		// --- 提问面板折叠（融合自 dsh-question-panel-collapse，MIT © Townrain） ---
		//
		// agent 提问时（ask_user / plan 审批等 question carrier 接管 composer），
		// 提问面板会占住对话框底部一大块。本组件在面板 header 内（✕ 取消按钮
		// 左边）提供纯图标 chevron 按钮：点它把面板折叠成一条紧凑小条（不是
		// 消失），再点展开，未提交的回答草稿原样保留（折叠只是 CSS 隐藏内容区，
		// 面板组件与草稿 state 始终存活在 composer chain 中）。折叠状态持久化
		// 到 localStorage。覆盖 QuestionFlow（[data-question-key]，有 header）
		// 与 PlanReviewPanel（[data-plan-review-key]，无 header，展开态退回
		// 悬浮按钮）两种面板形态。

		const QC_STORAGE_KEY = "dsh-tools:question-collapse:collapsed";
		const QC_CLASS_ON = "dsh-question-panel-collapse-on";
		const QC_PANEL_SELECTOR = "[data-question-key], [data-plan-review-key]";
		const QC_WRAPPER_CLASS = "dshqc-in-panel";

		function qcReadStored() {
			try { return window.localStorage.getItem(QC_STORAGE_KEY) === "1"; } catch (_) { return false; }
		}
		function qcWriteStored(on) {
			try { window.localStorage.setItem(QC_STORAGE_KEY, on ? "1" : "0"); } catch (_) { /* private mode */ }
		}

		// 模块级折叠状态 + 极简订阅集合：所有 entry 共享单一事实源，写时持久化。
		let qcCollapsed = qcReadStored();
		const qcListeners = new Set();

		function qcSetCollapsed(value) {
			qcCollapsed = value;
			qcWriteStored(value);
			for (const fn of qcListeners) fn(value);
		}

		/** React 桥：当前值 + 每次变化重渲染。 */
		function useQcCollapsed() {
			const [value, setValue] = React.useState(qcCollapsed);
			React.useEffect(() => {
				qcListeners.add(setValue);
				return () => qcListeners.delete(setValue);
			}, []);
			return value;
		}

		function QuestionCollapseControl() {
			const cfg = useConfig();
			const enabled = featureEnabled(cfg, "question.collapse");
			const [panelVisible, setPanelVisible] = React.useState(false);
			const [headerEl, setHeaderEl] = React.useState(null);
			const [wrapperEl, setWrapperEl] = React.useState(null);
			const [anchor, setAnchor] = React.useState(null);
			const on = useQcCollapsed();

			// 观察面板出现/消失 + composer seat 几何（rAF 节流）；开关关闭时不观察。
			React.useEffect(() => {
				...
				const update = () => {
					...
					raf = window.requestAnimationFrame(() => {
						raf = 0;
						const panel = document.querySelector(QC_PANEL_SELECTOR);
						const visible = panel !== null;
						if (visible !== lastVisible) {
							lastVisible = visible;
							setPanelVisible(visible);
							const header = visible ? panel.querySelector("header") : null;
							setHeaderEl(header);
						}
						const seat = document.querySelector("[data-composer-seat]");
						if (seat) {
							const rect = seat.getBoundingClientRect();
							setAnchor({ top: Math.round(rect.top), right: Math.max(0, Math.round(window.innerWidth - rect.right)) });
						}
					});
				};
				update();
			...
			// 把 portal 容器插进面板 header，紧贴 ✕ 按钮之前。
			React.useEffect(() => {
				if (headerEl === null) {
					setWrapperEl(null);
					return undefined;
				}
				let wrapper = headerEl.querySelector("." + QC_WRAPPER_CLASS);
				if (wrapper === null) {
					wrapper = document.createElement("div");
					wrapper.className = QC_WRAPPER_CLASS;
					const close = headerEl.querySelector("button");
					if (close !== null && close.parentNode === headerEl) headerEl.insertBefore(wrapper, close);
					else headerEl.appendChild(wrapper);
				}
				setWrapperEl(wrapper);
				return () => {
					if (wrapper !== null && wrapper.parentNode !== null) wrapper.parentNode.removeChild(wrapper);
				};
			}, [headerEl]);

			// 唯一的 DOM 副作用：在滚动容器上切 class，隐藏/显示面板内容区。
			React.useEffect(() => {
				if (typeof document === "undefined") return undefined;
				const root = document.querySelector("[data-conversation-scroll]");
				if (root) root.classList.toggle(QC_CLASS_ON, enabled && on);
				return undefined;
			}, [enabled, on]);

			// 所有 hooks 保持在条件返回之前（React hook 顺序规则）。
			const onToggle = React.useCallback(() => qcSetCollapsed(!on), [on]);
			if (!enabled) return null;
			if (!panelVisible) return null;

			// 面板在两种状态下都保留 header（折叠只隐藏 body/footer），按钮常驻，
			// chevron 翻转 ⌄（收起）/ ⌃（展开）。
			const chev = el("span", {
				className: on ? "dshqc-chev up" : "dshqc-chev",
				"aria-hidden": "true",
			});
			const title = on
				? "展开提问面板，继续回答（未提交的内容保留）"
				: "收起提问面板，对话框恢复为普通输入框（回答草稿不丢）";

			// Question 形态：按钮在面板 header 内（✕ 左边）。
			if (wrapperEl !== null) {
				const btn = el("button", {
					type: "button",
					className: "dshqc-inpanel-btn",
					"data-question-collapse-fab": "",
					title,
					"aria-label": title,
					"aria-pressed": on ? "true" : "false",
					onClick: onToggle,
				}, chev);
				return ReactDom.createPortal(btn, wrapperEl);
			}
			// 无 header 面板（plan review）：悬浮按钮贴在 seat 右上。
			if (anchor !== null) {
				return el("button", {
					type: "button",
					className: "dshqc-fab",
					"data-question-collapse-fab": "",
					style: {
						top: Math.max(0, anchor.top - 36) + "px",
						right: (anchor.right + 4) + "px",
					},
					title,
					"aria-label": title,
					"aria-pressed": on ? "true" : "false",
		...
		// 单元格的默认渲染：开启 ui.markdown 时用户消息气泡文本走 MarkdownText，
		// 关闭时回退纯文本投影（/name @name 引用 chip），外观与原生一致。
		...
		function markdownNodeRegistration(cfg) {
			if (cfg !== null && cfg !== undefined && cfg.features !== undefined && Array.isArray(cfg.features)) {
				const entry = cfg.features.find((f) => f.key === "ui.markdown");
		...
		// 位置/数量/悬挂存 dsh-tools featureConfig（ui.history）。details
		...
		function HistoryStrip(props) {
			...
			const t = props === undefined || props === null ? undefined : props.t;
			const cfg = useConfig();
			const hc = featureConfigOf(cfg, "ui.history");
		...
		function PinTurnAction(props) {
			...
			const togglePin = () => {
				...
				setFeatureConfig("ui.history", Object.assign({}, hc, { pinnedTurns: updated }));
			};
		...
		/** ui.history 的注册门：feature off 时注销 details/assistant-actions 条目。 */
		function historyRegistration(cfg) {
			if (cfg !== null && cfg !== undefined && cfg.features !== undefined && Array.isArray(cfg.features)) {
				const entry = cfg.features.find((f) => f.key === "ui.history");
				if (entry !== undefined && entry.enabled !== true) return false;
			}
			return true;
		}
		...
		function EnhancePanel() {
			...
			const save = (patch) => {
				setNotice(null);
				setFeatureConfig("ui.history", Object.assign({}, hc, patch)).then(() => {
					setNotice("历史条设置已保存");
				}, (error) => {
			...
			const markdownEntry = cfg === null ? undefined : (cfg.features || []).find((f) => f.key === "ui.markdown");
			const posOptions = [
				{ id: "off", label: "关闭" },
				{ id: "right", label: "右侧" },
				{ id: "left", label: "左侧" },
			...
			return el("div", { className: "dt-panel" },
				el("div", { className: "dt-panel-head" }, "界面增强"),
				...
				el("div", { className: "dt-enh-row" },
					el("div", { className: "dt-enh-text" },
						el("span", { className: "dt-name" }, "Markdown 渲染" + (markdownEntry !== undefined && markdownEntry.enabled === true ? "" : "（已停用）")),
						el("span", { className: "dt-meta" }, "开启后用户消息按 Markdown 渲染（标题/列表/代码块/@引用），在「功能开关」页启用。"),
					),
				),
				notice !== null ? el("div", { className: "dt-notice", role: "status" }, notice) : null,
			);
		...
		// --- 外观定制（融合自 dsh-client-ui-custom，MIT © Yoli-mi） ---
		//
		// 管线：featureConfig（已合并 defaultConfig）→ normalizeConfig（预设 ←
		// 显式值，类型收窄 + 钳制）→ applyConfig（写 --dsu-* 变量 + 门控开关）。
		// 改动即时生效；开关关闭时移除 data-dsu-active 门控，界面回原生。

		// --- color.ts 移植：纯色相数学（自动取色 / 和谐色板 / 随机灵感） ---
		...
		function apHslToHex(h, s, l) {
			const f = (n) => {
				const k = (n + h * 12) % 12;
				const a = s * Math.min(l, 1 - l);
				return l - a * Math.max(-1, Math.min(k - 3, 9 - k, 1));
			};
			return apRgbToHex(f(0) * 255, f(8) * 255, f(4) * 255);
		}

		/** 从主色派生和谐色板（Material-You 风）：基色 + 类比 + 互补 + 三角 + 暗色。 */
		function harmonySwatches(hex) {
			const rgb = apHexToRgb(hex);
			if (rgb === null) return [];
			const { h, s, l } = apRgbToHsl(rgb.r, rgb.g, rgb.b);
			if (s === 0) return [apRgbToHex(rgb.r, rgb.g, rgb.b)];
			const hue = h * 360;
			const wrap = (deg) => ((deg % 360) + 360) % 360;
			const at = (deg, sat, light) => apHslToHex(wrap(deg) / 360, sat, light);
			return [
				apRgbToHex(rgb.r, rgb.g, rgb.b),
				at(hue + 30, s, l),
				at(hue - 30, s, l),
				at(hue + 180, s, l),
				at(hue + 120, s, l),
		...
		/** 从 RGBA 像素数据提取主饱和色（canvas getImageData 输出）。 */
		function dominantColorFromRgba(data) {
			const HUE_BUCKETS = 24;
			const MIN_SATURATION = 0.18, MIN_LIGHTNESS = 0.14, MAX_LIGHTNESS = 0.86;
			const buckets = new Array(HUE_BUCKETS);
			for (let i = 0; i + 3 < data.length; i += 4) {
				const a = data[i + 3];
				if (a === undefined || a < 125) continue;
				const r = data[i] ?? 0, g = data[i + 1] ?? 0, b = data[i + 2] ?? 0;
				const { h, s, l } = apRgbToHsl(r, g, b);
			...
			return apRgbToHex(best.r / best.count, best.g / best.count, best.b / best.count);
		}

		/** 随机灵感主题（RNG 可注入以便测试）。 */
		function randomInspirationConfig(rng) {
			const rand = rng === undefined ? Math.random : rng;
			const pick = (options) => options[Math.min(options.length - 1, Math.floor(rand() * options.length))];
			const between = (lo, hi) => lo + rand() * (hi - lo);
			const INSPIRATION_HUES = [0, 10, 20, 32, 46, 62, 82, 104, 124, 148, 172, 196, 220, 244, 264, 286, 306, 328];
			const hue = pick(INSPIRATION_HUES) + between(-6, 6);
			const sat = between(0.30, 0.46);
			const light = between(0.52, 0.64);
			const dark = rand() < 0.35;
			const accent = apHslToHex(hue / 360, sat, light);
			const accentRgb = apHexToRgb(accent) ?? { r: 128, g: 128, b: 128 };
			const mid = apHslToHex(((hue + 16) % 360) / 360, Math.min(0.9, sat * 0.75), Math.min(0.92, light * 0.8 + 0.28));
			const midRgb = apHexToRgb(mid) ?? accentRgb;
			const end = apHexToRgb(dark ? apHslToHex(0, 0, 0.07) : apHslToHex(0, 0, 0.97)) ?? accentRgb;
			const gradient = dark
			...
			const surface = Math.round(between(30, 46));
			return {
				wallpaper: "",
				glass: pick(["light", "frosted", "mica"]),
				accent,
				autoAccent: false,
				...
				gradient,
				darkScrim: dark ? Math.round(between(26, 40)) : Math.round(between(10, 24)),
				fontFamily: "",
				codeFontFamily: "",
				fontScale: 1,
				scrollbarAccent: rand() < 0.7,
				vignette: dark,
				cornerRadius: pick(["md", "lg"]),
				surfaceShadow: pick(["soft", "medium"]),
				focusGlow: rand() < 0.6 ? "on" : "inherit",
		...
		const AP_ONE_OF_OPTIONS = {
			cornerRadius: ["inherit", "sm", "md", "lg", "xl"],
			surfaceShadow: ["inherit", "none", "soft", "medium", "strong"],
			focusGlow: ["inherit", "on"],
			wallpaperTone: ["inherit", "soft", "dim", "bright"],
		};

		function apClampNumber(value, lo, hi, fallback) {
			const n = typeof value === "number" && Number.isFinite(value) ? value : fallback;
			return Math.min(hi, Math.max(lo, n));
		}

		function apCleanString(value, fallback) {
			return typeof value === "string" && value.trim() !== "" ? value.trim() : fallback;
		}

		function apResolveGlass(raw) {
			const level = raw !== null && typeof raw === "object" && typeof raw.glass === "string" && Object.hasOwn(AP_GLASS_LEVELS, raw.glass)
				? raw.glass
				: AP_DEFAULTS.glass;
			const base = AP_GLASS_LEVELS[level];
			const explicit = typeof raw !== "undefined" && raw !== null && typeof raw.wallpaperBlur === "number"
				? apClampNumber(raw.wallpaperBlur, 0, 60, base.blur)
				: base.blur;
			return { blur: explicit, saturate: base.saturate };
		}

		function apToPercent(value, fallback) {
			return apClampNumber(value, 0, 100, fallback);
		}

		function apToVars(value) {
		...
		/** DEFAULTS ← 预设 ← 显式配置，逐字段收窄/钳制。纯函数，无 DOM。 */
		function normalizeConfig(raw, preset) {
			const merged = Object.assign({}, AP_DEFAULTS, preset, raw);
			const surfaceOpacity = apToPercent(merged.surfaceOpacity, AP_DEFAULTS.surfaceOpacity);
			const darkSurfaceOpacity = merged.darkSurfaceOpacity === undefined
				? surfaceOpacity
				: apToPercent(merged.darkSurfaceOpacity, surfaceOpacity);
			const glass = typeof merged.glass === "string" && Object.hasOwn(AP_GLASS_LEVELS, merged.glass) ? merged.glass : AP_DEFAULTS.glass;
			const { blur } = apResolveGlass(raw);
			const oneOf = (value, options, fallback) => {
				return typeof value === "string" && options.includes(value) ? value : fallback;
			};
			return {
				preset: apCleanString(merged.preset, AP_DEFAULTS.preset),
				wallpaper: apCleanString(merged.wallpaper, AP_DEFAULTS.wallpaper),
				wallpaperBlur: blur,
				glass,
				accent: apCleanString(merged.accent, AP_DEFAULTS.accent),
				autoAccent: typeof merged.autoAccent === "boolean" ? merged.autoAccent : AP_DEFAULTS.autoAccent,
				surfaceOpacity,
				sidebarOpacity: apToPercent(merged.sidebarOpacity, AP_DEFAULTS.sidebarOpacity),
				chatSurfaceOpacity: apToPercent(merged.chatSurfaceOpacity, AP_DEFAULTS.chatSurfaceOpacity),
				inputOpacity: apToPercent(merged.inputOpacity, AP_DEFAULTS.inputOpacity),
				codeBlockOpacity: apToPercent(merged.codeBlockOpacity, AP_DEFAULTS.codeBlockOpacity),
				darkSurfaceOpacity,
				gradient: typeof merged.gradient === "string" && merged.gradient.trim() !== "" ? merged.gradient.trim() : "",
				darkScrim: apToPercent(merged.darkScrim, AP_DEFAULTS.darkScrim),
				...
				fontScale: Math.round(apClampNumber(merged.fontScale, 0.9, 1.1, AP_DEFAULTS.fontScale) * 20) / 20,
				scrollbarAccent: typeof merged.scrollbarAccent === "boolean" ? merged.scrollbarAccent : AP_DEFAULTS.scrollbarAccent,
				vignette: typeof merged.vignette === "boolean" ? merged.vignette : AP_DEFAULTS.vignette,
				cornerRadius: oneOf(merged.cornerRadius, AP_ONE_OF_OPTIONS.cornerRadius, AP_DEFAULTS.cornerRadius),
				surfaceShadow: oneOf(merged.surfaceShadow, AP_ONE_OF_OPTIONS.surfaceShadow, AP_DEFAULTS.surfaceShadow),
				focusGlow: oneOf(merged.focusGlow, AP_ONE_OF_OPTIONS.focusGlow, AP_DEFAULTS.focusGlow),
				wallpaperTone: oneOf(merged.wallpaperTone, AP_ONE_OF_OPTIONS.wallpaperTone, AP_DEFAULTS.wallpaperTone),
				darkAccent: apCleanString(merged.darkAccent, AP_DEFAULTS.darkAccent),
				customCss: typeof merged.customCss === "string" ? merged.customCss : "",
				customVars: apToVars(merged.customVars),
			};
		}
		...
		const AP_PRESETS = [
			{ id: "ink-teal", name: "黛青", description: "青玉色渐变，静谧沉稳。", config: { wallpaper: "", glass: "light", accent: "#1e8f7e", autoAccent: false, surfaceOpacity: 40, sidebarOpacity: 40, chatSurfaceOpacity: 64, inputOpacity: 70, codeBlockOpacity: 50, darkSurfaceOpacity: 40, gradient: "linear-gradient(160deg, rgb(30 143 126 / 0.42) 0%, rgb(103 197 178 / 0.26) 55%, rgb(243 250 247 / 0.34) 100%)", darkScrim: 18, fontFamily: "", scrollbarAccent: true, vignette: false, cornerRadius: "lg", surfaceShadow: "soft", focusGlow: "on", wallpaperTone: "inherit", darkAccent: "" } },
			{ id: "ink-blue", name: "黛蓝", description: "黛蓝渐变，深邃克制的蓝。", config: { wallpaper: "", glass: "light", accent: "#3f63d8", autoAccent: false, surfaceOpacity: 38, sidebarOpacity: 38, chatSurfaceOpacity: 62, inputOpacity: 68, codeBlockOpacity: 48, darkSurfaceOpacity: 38, gradient: "linear-gradient(160deg, rgb(63 99 216 / 0.40) 0%, rgb(129 158 233 / 0.24) 55%, rgb(246 248 253 / 0.34) 100%)", darkScrim: 18, fontFamily: "", scrollbarAccent: true, vignette: false, cornerRadius: "md", surfaceShadow: "soft", focusGlow: "on", wallpaperTone: "inherit", darkAccent: "" } },
			{ id: "dusty-rose", name: "藕荷", description: "藕荷色渐变，温润柔和的粉。", config: { wallpaper: "", glass: "light", accent: "#c2788f", autoAccent: false, surfaceOpacity: 42, sidebarOpacity: 42, chatSurfaceOpacity: 66, inputOpacity: 70, codeBlockOpacity: 52, darkSurfaceOpacity: 42, gradient: "linear-gradient(160deg, rgb(194 120 143 / 0.36) 0%, rgb(224 168 186 / 0.22) 55%, rgb(252 246 248 / 0.32) 100%)", darkScrim: 16, fontFamily: "", scrollbarAccent: true, vignette: false, cornerRadius: "md", surfaceShadow: "soft", focusGlow: "inherit", wallpaperTone: "inherit", darkAccent: "" } },
			{ id: "apricot-gold", name: "杏金", description: "杏金色渐变，温雅低调的金。", config: { wallpaper: "", glass: "light", accent: "#c0863c", autoAccent: false, surfaceOpacity: 42, sidebarOpacity: 42, chatSurfaceOpacity: 66, inputOpacity: 72, codeBlockOpacity: 52, darkSurfaceOpacity: 42, gradient: "linear-gradient(160deg, rgb(192 134 60 / 0.36) 0%, rgb(224 184 122 / 0.22) 55%, rgb(250 246 238 / 0.32) 100%)", darkScrim: 16, fontFamily: "", scrollbarAccent: true, vignette: false, cornerRadius: "md", surfaceShadow: "soft", focusGlow: "inherit", wallpaperTone: "inherit", darkAccent: "" } },
			{ id: "mist-gray", name: "雾灰", description: "雾灰色渐变，清冷安静的灰蓝。", config: { wallpaper: "", glass: "frosted", accent: "#64728e", autoAccent: false, surfaceOpacity: 30, sidebarOpacity: 30, chatSurfaceOpacity: 52, inputOpacity: 60, codeBlockOpacity: 40, darkSurfaceOpacity: 30, gradient: "linear-gradient(160deg, rgb(100 114 142 / 0.34) 0%, rgb(44 52 72 / 0.38) 55%, rgb(16 19 27 / 0.48) 100%)", darkScrim: 26, fontFamily: "", scrollbarAccent: false, vignette: true, cornerRadius: "md", surfaceShadow: "medium", focusGlow: "inherit", wallpaperTone: "inherit", darkAccent: "" } },
			{ id: "ink-violet", name: "墨紫", description: "墨紫色渐变，沉静神秘。", config: { wallpaper: "", glass: "frosted", accent: "#8268c4", autoAccent: false, surfaceOpacity: 28, sidebarOpacity: 28, chatSurfaceOpacity: 50, inputOpacity: 60, codeBlockOpacity: 40, darkSurfaceOpacity: 28, gradient: "linear-gradient(160deg, rgb(130 104 196 / 0.36) 0%, rgb(76 62 122 / 0.42) 55%, rgb(22 18 38 / 0.52) 100%)", darkScrim: 30, fontFamily: "", scrollbarAccent: true, vignette: true, cornerRadius: "lg", surfaceShadow: "medium", focusGlow: "on", wallpaperTone: "inherit", darkAccent: "#8268c4" } },
		];

		function resolvePreset(id) {
			if (id === undefined || id === "") return undefined;
			for (const preset of AP_PRESETS) {
				if (preset.id === id) return preset.config;
			}
			return undefined;
		}

		// --- apply.ts 移植：config → DOM ---

		const AP_CUSTOM_STYLE_ID = "dsh-tools-ui-appearance-css-user";
		const AP_CORNER_RADIUS_PX = { sm: 6, md: 10, lg: 14, xl: 18 };
		const AP_SURFACE_SHADOW_CSS = {
			none: "none",
			soft: "0 8px 24px rgb(0 0 0 / 0.10)",
			medium: "0 14px 36px rgb(0 0 0 / 0.16)",
			strong: "0 24px 56px rgb(0 0 0 / 0.26)",
		};
		const AP_WALLPAPER_TONE_CSS = {
		...
		const apEscapeUrl = (text) => text.replaceAll('"', '\\"');

		/** 壁纸 → 离屏 canvas 提取主色（CORS/解码失败返回 null）。 */
		function extractWallpaperAccent(url) {
			try {
				return new Promise((resolve) => {
					...
					image.onload = () => {
						try {
							...
							resolve(dominantColorFromRgba(pixels));
						} catch {
							resolve(null);
						}
					};
		...
		/** 完全中性（= 默认）时门控关闭，界面与原生逐字节一致。 */
		function apIsNeutralConfig(config) {
			return config.wallpaper === AP_DEFAULTS.wallpaper
				&& config.gradient === AP_DEFAULTS.gradient
				&& config.accent === AP_DEFAULTS.accent
				&& config.autoAccent === AP_DEFAULTS.autoAccent
				&& config.glass === AP_DEFAULTS.glass
				&& config.surfaceOpacity === AP_DEFAULTS.surfaceOpacity
				&& config.sidebarOpacity === AP_DEFAULTS.sidebarOpacity
				&& config.chatSurfaceOpacity === AP_DEFAULTS.chatSurfaceOpacity
				&& config.inputOpacity === AP_DEFAULTS.inputOpacity
				&& config.codeBlockOpacity === AP_DEFAULTS.codeBlockOpacity
				&& config.darkSurfaceOpacity === 100
				&& config.darkScrim === AP_DEFAULTS.darkScrim
				&& config.fontFamily === AP_DEFAULTS.fontFamily
				&& config.codeFontFamily === AP_DEFAULTS.codeFontFamily
				&& config.fontScale === AP_DEFAULTS.fontScale
				&& config.scrollbarAccent === AP_DEFAULTS.scrollbarAccent
				&& config.vignette === AP_DEFAULTS.vignette
				&& config.cornerRadius === AP_DEFAULTS.cornerRadius
				&& config.surfaceShadow === AP_DEFAULTS.surfaceShadow
				&& config.focusGlow === AP_DEFAULTS.focusGlow
				&& config.wallpaperTone === AP_DEFAULTS.wallpaperTone
				&& config.darkAccent === AP_DEFAULTS.darkAccent
				&& config.customCss === AP_DEFAULTS.customCss
				&& Object.keys(config.customVars).length === 0;
		}

		/** 归一化配置 → DOM 效果（--dsu-* 变量 + 门控 + customCss/customVars）。 */
		function applyConfig(config) {
			const root = document.documentElement;
			if (apIsNeutralConfig(config)) {
				root.removeAttribute("data-dsu-active");
				const style = document.getElementById(AP_CUSTOM_STYLE_ID);
				if (style !== null) style.remove();
				return;
			}
			root.setAttribute("data-dsu-active", "1");
			const set = (name, value) => root.style.setProperty(name, value);
			const wallpaper = apCleanString(config.wallpaper, "");
			if (wallpaper === "") root.style.removeProperty("--dsu-wallpaper");
			else set("--dsu-wallpaper", 'url("' + apEscapeUrl(wallpaper) + '")');
			set("--dsu-blur", apClampNumber(config.wallpaperBlur, 0, 60, 14) + "px");
			set("--dsu-saturate", String((AP_GLASS_LEVELS[config.glass] || AP_GLASS_LEVELS.frosted).saturate));
			set("--dsu-accent", apCleanString(config.accent, "#4176e6"));
			set("--dsu-surface-alpha", apClampNumber(config.surfaceOpacity, 0, 100, 50) + "%");
			set("--dsu-sidebar-alpha", apClampNumber(config.sidebarOpacity, 0, 100, 50) + "%");
			set("--dsu-chat-alpha", apClampNumber(config.chatSurfaceOpacity, 0, 100, 80) + "%");
			set("--dsu-input-alpha", apClampNumber(config.inputOpacity, 0, 100, 82) + "%");
			set("--dsu-code-alpha", apClampNumber(config.codeBlockOpacity, 0, 100, 45) + "%");
			set("--dsu-dark-alpha", apClampNumber(config.darkSurfaceOpacity, 0, 100, config.surfaceOpacity) + "%");
			set("--dsu-scrim", "rgb(15 17 21 / " + (apClampNumber(config.darkScrim, 0, 100, 22) / 100) + ")");
			set("--dsu-gradient", config.gradient !== "" ? config.gradient : "none");
			const font = apCleanString(config.fontFamily, "");
			if (font !== "") set("--dsu-font", font);
			else root.style.removeProperty("--dsu-font");
			const codeFont = apCleanString(config.codeFontFamily, "");
			if (codeFont !== "") set("--dsu-code-font", codeFont);
			else root.style.removeProperty("--dsu-code-font");
			if (config.fontScale !== 1) set("--dsu-font-scale", String(apClampNumber(config.fontScale, 0.9, 1.1, 1)));
			...
			if (config.cornerRadius !== "inherit") set("--dsu-radius", (AP_CORNER_RADIUS_PX[config.cornerRadius] ?? 10) + "px");
			else root.style.removeProperty("--dsu-radius");
			if (config.surfaceShadow !== "inherit") set("--dsu-shadow", AP_SURFACE_SHADOW_CSS[config.surfaceShadow] ?? "none");
			else root.style.removeProperty("--dsu-shadow");
			set("--dsu-focus-glow", config.focusGlow === "on" ? "1" : "0");
			if (config.wallpaperTone !== "inherit") set("--dsu-tone", AP_WALLPAPER_TONE_CSS[config.wallpaperTone] ?? "none");
			else root.style.removeProperty("--dsu-tone");
			const darkAccent = apCleanString(config.darkAccent, "");
			if (darkAccent !== "") set("--dsu-dark-accent", darkAccent);
			else root.style.removeProperty("--dsu-dark-accent");
			if (config.autoAccent && wallpaper !== "") {
				extractWallpaperAccent(wallpaper).then((color) => {
					if (color !== null) root.style.setProperty("--dsu-accent", color);
				});
			}
			for (const key of Object.keys(config.customVars)) {
				const value = config.customVars[key];
				if (value === "") root.style.removeProperty(key);
				else root.style.setProperty(key, value);
			}
			let style = document.getElementById(AP_CUSTOM_STYLE_ID);
			if (config.customCss !== "") {
				if (style === null) {
					style = document.createElement("style");
					style.id = AP_CUSTOM_STYLE_ID;
					style.dataset.plugin = "dsh-tools";
		...
		/**
		 * 从配置快照应用外观主题；feature 关闭时移除门控（回原生）。
		 * 作为 configListeners 的 listener 被无参调用（setConfig 统一
		 * `listener()`），必须从 configStore 取当前快照——接收外部参数会
		 * 恒为 undefined 而永远走关闭分支（v0.7.0 实测 bug）。
		 */
		function applyAppearanceTheme() {
			if (typeof document === "undefined" || document.documentElement === undefined) return;
			const cfg = configStore;
			if (cfg === null || cfg === undefined || !featureEnabled(cfg, "ui.appearance")) {
				document.documentElement.removeAttribute("data-dsu-active");
				const style = document.getElementById(AP_CUSTOM_STYLE_ID);
				if (style !== null) style.remove();
				return;
			}
			const raw = featureConfigOf(cfg, "ui.appearance");
			applyConfig(normalizeConfig(raw, resolvePreset(raw.preset)));
		}

		// --- 外观设置面板（改动即保存，保存即生效） ---

		const AP_GLASS_LABELS = { off: "不透明", light: "轻玻璃", frosted: "毛玻璃", mica: "云母" };
		const AP_CORNER_LABELS = { inherit: "跟随默认", sm: "小圆角", md: "中圆角", lg: "大圆角", xl: "超大圆角" };
		const AP_SHADOW_LABELS = { inherit: "跟随默认", none: "无阴影", soft: "柔和", medium: "中等", strong: "强烈" };
		const AP_TONE_LABELS = { inherit: "不叠加", soft: "柔和", dim: "变暗", bright: "提亮" };

		function AppearancePanel() {
			const cfg = useConfig();
			const [notice, setNotice] = React.useState(null);
			const [draftCss, setDraftCss] = React.useState("");
			const [draftVars, setDraftVars] = React.useState("");
			const ac = featureConfigOf(cfg, "ui.appearance");
			const [cfgTick, setCfgTick] = React.useState(0);
			React.useEffect(() => {
				setDraftCss(typeof ac.customCss === "string" ? ac.customCss : "");
				setDraftVars(typeof ac.customVars === "object" && ac.customVars !== null ? JSON.stringify(ac.customVars, null, 2) : "");
				setCfgTick((n) => n + 1);
			}, [cfg]);

			const save = (patch) => {
				setNotice(null);
				setFeatureConfig("ui.appearance", Object.assign({}, ac, patch)).then(() => {
					setNotice("已保存并即时生效");
				}, (error) => {
					setNotice("保存失败：" + String((error && error.message) || error));
				});
			};
			const saveCss = () => save({ customCss: draftCss });
			const saveVars = () => {
				let parsed = {};
				if (draftVars.trim() !== "") {
					try {
						parsed = JSON.parse(draftVars);
					} catch {
						setNotice("customVars 不是合法 JSON");
						return;
					}
					if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) {
						setNotice("customVars 需为对象");
						return;
					}
				}
				save({ customVars: parsed });
			};
			const applyPreset = (id) => {
				const preset = resolvePreset(id);
				const merged = Object.assign({}, ac, preset, { preset: id });
				setFeatureConfig("ui.appearance", merged).then(() => {
					setNotice("已应用预设「" + (AP_PRESETS.find((p) => p.id === id) || { name: id }).name + "」，可继续微调");
				}, (error) => {
					setNotice("保存失败：" + String((error && error.message) || error));
				});
			};
			const randomIdea = () => {
				const idea = randomInspirationConfig();
				setFeatureConfig("ui.appearance", Object.assign({}, ac, idea, { preset: "" })).then(() => {
					setNotice("已生成随机灵感配色");
				}, (error) => {
					setNotice("保存失败：" + String((error && error.message) || error));
				});
			...
			const num = (key) => (typeof ac[key] === "number" ? ac[key] : 100);
			const bool = (key) => ac[key] === true;
			const slider = (key, label, min, max, step) => el("div", { className: "dt-ap-field" },
				el("div", { className: "dt-ap-field-head" },
					el("span", { className: "dt-ap-label" }, label),
					el("span", { className: "dt-ap-value" }, String(num(key)) + "%"),
				),
				el("input", {
					type: "range",
					min, max, step,
					value: num(key),
					onChange: (event) => save(Object.assign({}, { [key]: Number(event.target.value) })),
				}),
			);
			const toggle = (key, label) => el("label", { className: "dt-ap-toggle" },
				el("input", {
					type: "checkbox",
					checked: bool(key),
					onChange: (event) => save(Object.assign({}, { [key]: event.target.checked })),
				}),
				el("span", null, label),
			);
			const select = (key, label, options, labels) => el("div", { className: "dt-ap-field" },
				el("div", { className: "dt-ap-field-head" },
					el("span", { className: "dt-ap-label" }, label),
					el("select", {
						className: "dt-ap-select",
						value: typeof ac[key] === "string" ? ac[key] : "inherit",
						onChange: (event) => save(Object.assign({}, { [key]: event.target.value })),
					},
						options.map((option) => el("option", { value: option, key: option }, (labels || {})[option] || option)),
					),
				),
			);
			const text = (key, label, placeholder) => el("div", { className: "dt-ap-field" },
				el("div", { className: "dt-ap-field-head" },
					el("span", { className: "dt-ap-label" }, label),
					el("input", {
						type: "text",
						...
						placeholder,
						defaultValue: typeof ac[key] === "string" ? ac[key] : "",
						key: key + ":" + cfgTick,
						onBlur: (event) => save(Object.assign({}, { [key]: event.target.value })),
					}),
				),
			);
			const accentInput = el("div", { className: "dt-ap-field" },
				el("div", { className: "dt-ap-field-head" },
					el("span", { className: "dt-ap-label" }, "强调色"),
					el("span", { className: "dt-ap-accent-row" },
						el("input", {
							type: "color",
							className: "dt-ap-color",
							value: typeof ac.accent === "string" && /^#[0-9a-f]{6}$/i.test(ac.accent) ? ac.accent : "#4176e6",
							onChange: (event) => save({ accent: event.target.value }),
						}),
						el("button", { type: "button", className: "dt-btn", onClick: randomIdea }, "随机灵感"),
					),
				),
				el("div", { className: "dt-ap-swatches" },
					harmonySwatches(typeof ac.accent === "string" ? ac.accent : "#4176e6").map((swatch) => el("button", {
						key: swatch,
						type: "button",
						className: "dt-ap-swatch" + (ac.accent === swatch ? " active" : ""),
						style: { background: swatch },
						title: swatch,
						"aria-label": "强调色 " + swatch,
						onClick: () => save({ accent: swatch }),
					})),
				),
			);

			return el("div", { className: "dt-panel" },
				el("div", { className: "dt-panel-head" }, "外观定制"),
				el("p", { className: "dt-meta" }, "所有改动保存后立即生效（无需重启）。关闭「外观」开关即恢复原生界面。"),
				el("div", { className: "dt-ap-field" },
					el("div", { className: "dt-ap-field-head" },
						el("span", { className: "dt-ap-label" }, "预设"),
						el("button", {
							type: "button",
							className: "dt-btn" + (typeof ac.preset === "string" && ac.preset !== "" ? "" : " primary"),
							onClick: () => save({ preset: "" }),
						}, "无预设"),
					),
					el("div", { className: "dt-ap-presets" },
						AP_PRESETS.map((preset) => el("button", {
							key: preset.id,
							type: "button",
							className: "dt-ap-preset" + (ac.preset === preset.id ? " active" : ""),
							onClick: () => applyPreset(preset.id),
						},
							el("span", { className: "dt-ap-preset-name" }, preset.name),
							el("span", { className: "dt-ap-preset-desc" }, preset.description),
						)),
					),
				),
				text("wallpaper", "壁纸", "壁纸 URL 或 Web 可访问路径（留空不使用）"),
				select("glass", "玻璃档位", ["off", "light", "frosted", "mica"], AP_GLASS_LABELS),
				slider("wallpaperBlur", "模糊半径", 0, 60, 1),
				accentInput,
				toggle("autoAccent", "从壁纸自动派生强调色（成功后覆盖手动色）"),
				slider("surfaceOpacity", "主表面不透明度", 0, 100, 1),
				slider("sidebarOpacity", "侧栏不透明度", 0, 100, 1),
				slider("chatSurfaceOpacity", "聊天列不透明度", 0, 100, 1),
				slider("inputOpacity", "输入框不透明度", 0, 100, 1),
				slider("codeBlockOpacity", "代码块不透明度", 0, 100, 1),
				slider("darkSurfaceOpacity", "暗色模式表面不透明度", 0, 100, 1),
				text("gradient", "亮色渐变", "叠加在壁纸上的渐变（CSS，留空无）"),
				slider("darkScrim", "暗色遮罩强度", 0, 100, 1),
				text("fontFamily", "界面字体", "字体栈覆盖（留空 = 主题默认）"),
				text("codeFontFamily", "代码字体", "代码字体栈覆盖（留空 = 主题默认）"),
				el("div", { className: "dt-ap-field" },
					el("div", { className: "dt-ap-field-head" },
						el("span", { className: "dt-ap-label" }, "全局字号缩放"),
						el("span", { className: "dt-ap-value" }, (typeof ac.fontScale === "number" ? ac.fontScale : 1) + "×"),
					),
					el("input", {
						type: "range",
						min: 0.9, max: 1.1, step: 0.05,
						value: typeof ac.fontScale === "number" ? ac.fontScale : 1,
						onChange: (event) => save({ fontScale: Number(event.target.value) }),
					}),
				),
				toggle("scrollbarAccent", "滚动条使用强调色"),
				toggle("vignette", "内嵌晕影"),
				select("cornerRadius", "圆角", ["inherit", "sm", "md", "lg", "xl"], AP_CORNER_LABELS),
				select("surfaceShadow", "表面阴影", ["inherit", "none", "soft", "medium", "strong"], AP_SHADOW_LABELS),
				select("focusGlow", "焦点光晕", ["inherit", "on"], { inherit: "跟随默认", on: "开启" }),
				select("wallpaperTone", "壁纸色调", ["inherit", "soft", "dim", "bright"], AP_TONE_LABELS),
				text("darkAccent", "暗色强调色", "暗色模式强调色覆盖（留空 = 继承主色）"),
				el("div", { className: "dt-ap-field" },
					el("div", { className: "dt-ap-field-head" }, el("span", { className: "dt-ap-label" }, "自定义 CSS（逃生舱）")),
					el("textarea", {
						className: "dt-ap-textarea",
						rows: 4,
						placeholder: "原样追加到页面的 CSS",
						value: draftCss,
				...
				el("div", { className: "dt-ap-field" },
					el("div", { className: "dt-ap-field-head" }, el("span", { className: "dt-ap-label" }, "自定义变量（customVars，JSON）")),
					el("textarea", {
						className: "dt-ap-textarea",
						rows: 4,
						placeholder: '{\n  "--my-var": "value"\n}',
						value: draftVars,
						onChange: (event) => setDraftVars(event.target.value),
						onBlur: saveVars,
					}),
				),
				notice !== null ? el("div", { className: "dt-notice", role: "status" }, notice) : null,
		...
		function apply(pluginCtx) {
			ctx = pluginCtx;
			// 外观主题（ui.appearance）：配置变化即重应用（--dsu-* 变量写入
			// <html>），开关关闭移除门控回原生。无 slot，纯全局副作用。
			applyAppearanceTheme();
			configListeners.add(applyAppearanceTheme);
			...
			// 提问面板折叠（question.collapse）：组件内部按 featureEnabled 自判，
			// 开关关闭时渲染 null 且不注入 CSS 副作用之外的行为。
			ctx.slots.inject("shell.overlay", () => ctx.slots.register(
				{ name: "shell.overlay", id: "question-collapse", order: 90, label: "提问面板折叠" },
				() => React.createElement(QuestionCollapseControl, null),
			));
			// 用户消息 Markdown 渲染（ui.markdown）：keyed slot 注册/注销随功能
			...
			// 浮动历史条（ui.history）：details（priority -1，覆盖右侧栏）与
			...
			return () => {
				configListeners.delete(applyAppearanceTheme);
				ctx = undefined;
			};
		}
		...
		exports.__dshToolsTest = { evaluateTurnDone, nativeNoticePayload, tabModel, updateCheckDecision, deleteChatLoadDecision, groupSessionsByWorkspace, formatBytes, markdownNodeRegistration, historyRegistration, buildTurns, mergeVisibleTurns, decodeUsageRow, aggregateUsage, usageByBucket, usageModelKeys, formatTokens, formatDuration, normalizeConfig, resolvePreset, harmonySwatches, dominantColorFromRgba, randomInspirationConfig, UsagePanel, catalogFilter, catalogCounts, catalogModuleShortName, catalogTabRegistration, catalogFetchGate };
