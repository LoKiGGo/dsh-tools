...
// 会话目录大小磁盘缓存：避免每次打开会话管理都全量递归 stat 所有会话目录。
// 以 session 目录绝对路径为键；命中条件 = 目录 mtime + 主日志文件 size/mtime 均未变。
const SIZE_CACHE_FILE = () => join(pluginsDataDir(), "dsh-tools-delete-chat-sizes.json");
let sizeCache = {};
let sizeCacheLoaded = false;
let sizeCacheDirty = false;

async function loadSizeCache() {
	if (sizeCacheLoaded) return sizeCache;
	sizeCache = {};
	try {
		const text = await readFile(SIZE_CACHE_FILE(), "utf8");
		const parsed = JSON.parse(text);
		if (parsed !== null && typeof parsed === "object" && parsed.sessions !== null && typeof parsed.sessions === "object") {
			sizeCache = parsed.sessions;
		}
	} catch {
		// 首次运行/缓存损坏：按空缓存处理。
	}
...
async function saveSizeCache() {
	...
	try {
		mkdirSync(pluginsDataDir(), { recursive: true });
		await writeFile(SIZE_CACHE_FILE(), JSON.stringify({ sessions: sizeCache }, null, 2), "utf8");
		sizeCacheDirty = false;
	} catch {
		// 缓存写入失败不应拖垮会话列表；下次会重算。
	}
}

function invalidateSizeCacheDir(dir) {
	if (!sizeCacheLoaded) return;
	if (sizeCache[dir] !== undefined) {
		delete sizeCache[dir];
		sizeCacheDirty = true;
	}
}
 ...
 * is the whole session directory. The directory name is the encoded session id
 * (may or may not start with `session-`, e.g. WeChat room/per-user sessions use
 * IDs like `__room__` or `wxid_...`), so only the direct child file name is
 * used to recognize a session log — that is enough to avoid deleting arbitrary
 * directories.
 */
...
export function register(ctx, api) {
	...
	async function listSessionsJson(includeSizes = true) {
		...
		// v0.10：`includeSizes=false` 时跳过大小计算，客户端先快速渲染再异步补齐。
		...
		if (includeSizes) {
			...
			const locateRecord = (record) => {
				...
				try {
					...
					return { dir, path: location.path };
				} catch {
					return null;
				}
			};
			const sessionSizes = await mapConcurrent(records, 8, async (record) => {
				const located = locateRecord(record);
				if (located === null) return { bytes: 0, unknown: false }; // 无目录：占用 0
				const { dir, path } = located;
				// 磁盘缓存命中：目录 mtime 与主日志文件 size/mtime 都没变，直接复用大小。
				const entry = cache[dir];
				if (entry !== undefined) {
					try {
						const [dirStat, fileStat] = await Promise.all([stat(dir), stat(path)]);
						if (dirStat.mtimeMs === entry.dirMtimeMs && fileStat.size === entry.mainSize && fileStat.mtimeMs === entry.mainMtimeMs) {
							return { bytes: entry.bytes, unknown: false };
						}
					} catch {
						// stat 失败按未命中处理，重新 dirSize（dirSize 内部有容错）。
					}
				}
				...
				if (bytes !== null) {
					try {
						const [dirStat, fileStat] = await Promise.all([stat(dir), stat(path)]);
						cache[dir] = { bytes, dirMtimeMs: dirStat.mtimeMs, mainSize: fileStat.size, mainMtimeMs: fileStat.mtimeMs };
						sizeCacheDirty = true;
					} catch {
						// 无法读取 stat 就不写缓存，下次继续尝试。
					}
				}
			...
			if (sizeCacheDirty) await saveSizeCache();
		}
		...
		return records.map((record) => {
			...
			return {
				...
				sizeBytes: includeSizes ? (entry.unknown ? null : entry.bytes) : null,
				workspace: wsBrief === null
					? null
					: {
						...wsBrief,
						sizeBytes: includeSizes ? (wsFull === undefined ? null : workspaceChatSizeOf(wsFull)) : null,
					},
			};
		});
	}
	...
	async function deleteSession(sessionId, confirmLive) {		if (typeof sessionId !== "string" || sessionId.length === 0) return { ok: false, error: "bad-request" };
		...
		if (outcome.exitCode === 0) {
			await loadSizeCache();
			invalidateSizeCacheDir(dir);
			await saveSizeCache();
			return { ok: true, live: record.live };
		}
	...
	const dispose = ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			try {
				...
				if (method === "list") {
					const includeSizes = payload === null || payload === undefined || payload.includeSizes !== false;
					api.writeOk(res, await listSessionsJson(includeSizes));
					return;
				}
				if (method === "delete") {
