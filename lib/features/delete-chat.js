...
export function register(ctx, api) {
	...
	async function listSessionsJson() {
		...
		// 存储占用（v0.7.0）：会话大小 = 其 session-<id> 目录大小（与删除语义一致）；
		// 工作区大小 = 工作区目录大小。并发限 8 路；不可读目录为 null，不阻塞列表。
		...
		const workspaceSizes = await mapConcurrent(workspaceList, 8, (ws) => dirSize(ws.path));
		return records.map((record, index) => {
			const title = titleById.get(record.header.id);
			const ws = workspaceBySession.get(record.header.id) ?? null;
			const wsIndex = ws === null ? -1 : workspaceList.findIndex((item) => item.id === ws.id);
			return {
				id: record.header.id,
				...
				sizeBytes: sizeBySession.get(record.header.id) ?? null,
				workspace: ws === null
					? null
					: {
						...ws,
						sizeBytes: wsIndex >= 0 ? (workspaceSizes[wsIndex] ?? null) : null,
					},
			};
		});
	}
