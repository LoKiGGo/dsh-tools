...
import { githubUrlOf, parseGithubSpec, githubRepoDescription, npmPackageDescription, orderDescriptions } from "./github-ref.js";
...
function installedSnapshot() {
	...
	for (const pkgName of Object.keys(dependencies)) {
		...
		const isBundle = pkg !== null && pkg.dsh?.bundle?.patch !== undefined;
		const description = pkg !== null && typeof pkg.description === "string" ? pkg.description : "";
...
/**
 * Collect description candidates per plugin and order them Chinese-first:
 * - the local package.json description always counts as a candidate;
 * - only when the local description is missing: github: specs probe the
 *   GitHub API, registry-style specs probe the npm registry metadata;
 * - any probe failure silently keeps that candidate absent.
 * `descriptions` carries every distinct candidate (zh first); `description`
 * is the preferred single one (the first entry).
 */
async function enrichDescriptions(plugins) {
	const probes = [];
	for (const plugin of plugins) {
		const local = typeof plugin.description === "string" ? plugin.description.trim() : "";
		plugin.descriptions = local === "" ? [] : [local];
		if (local !== "") continue;
		const ref = parseGithubSpec(plugin.spec);
		if (ref !== null) {
			probes.push(githubRepoDescription(ref.owner, ref.repo).then((description) => {
				if (description !== "") plugin.descriptions.push(description);
			}));
		} else if (/^[^/:]+$/.test(String(plugin.spec ?? "").trim())) {
			// bare name or semver range → registry-installed → npm registry metadata
			probes.push(npmPackageDescription(plugin.name).then((description) => {
				if (description !== "") plugin.descriptions.push(description);
			}));
		}
	}
	...
	for (const plugin of plugins) {
		plugin.descriptions = orderDescriptions(plugin.descriptions ?? []);
		plugin.description = plugin.descriptions.length > 0 ? plugin.descriptions[0] : "";
	}
	return plugins;
}
		...
		handler: async (req, res) => {
			...
			try {
				...
				if (method === "list") {
					...
					await enrichDescriptions(snapshot.plugins);
					api.writeOk(res, snapshot);
					return;
				}
				if (method === "set") {
