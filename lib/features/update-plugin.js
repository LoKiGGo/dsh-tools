 ...
 * GitHub 安装（`github:owner/repo`、`git+https://github.com/...` 或
 * github.com 下载/archive URL）用 GitHub Releases API 探测最新 tag
 * （无 Release 时回退 tags 列表）；
 * 其余本地/远程引用（link:/file:/workspace:/非 GitHub 的 git+:/其他 URL）跳过。
...
import { parseGithubSpec as parseGitHubSpec, githubUrlOf, versionFromTag, githubLatestTag, compareVersions } from "./github-ref.js";
...
function classifySpec(spec) {
	...
	if (/^git\+https:\/\/(www\.)?github\.com\//i.test(text)) return "github";
...
function buildUpdateSpec(spec, tag) {
	...
	const text = String(spec ?? "").trim();
	if (/^github:/i.test(text)) return `github:${ref.owner}/${ref.repo}#${tag}`;
	if (/^git\+https:\/\/(www\.)?github\.com\//i.test(text)) return `git+https://github.com/${ref.owner}/${ref.repo}.git#${tag}`;
