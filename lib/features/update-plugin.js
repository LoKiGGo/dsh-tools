 ...
 *
 * 数据面路由前缀：/dsh-tools/update-plugin/api/{check,update}。对 profile
 ...
 * GitHub archive tarball URL。更新在重启 dsh web 后生效。
 */
...
function parseGitHubSpec(spec) {
	...
	if (/^github:/i.test(text)) {
		...
		const m = /^https?:\/\/(?:www\.)?github\.com\/([^/]+)\/([^/#?]+)/i.exec(text);
...
function installedSnapshot() {
	...
	for (const pkgName of Object.keys(dependencies)) {
		...
		let version = "";
		try {
			version = readJson(join(packageDir(pkgName), "package.json")).version ?? "";
		} catch {}
		entries.push({
			name: pkgName,
			spec,
...
export { classifySpec, parseGitHubSpec, versionFromTag, buildUpdateSpec, githubLatestTag };
