 ...
 *   - 白名单、会话模式等基础配置
 *
 * 网关逻辑在 lib/wechat/gateway.js（参考 dsh-weixin-gateway 改写并加白名单），
...
} from "../wechat/vendor/weixin/accounts.js";
import { acquireRunLock, releaseRunLock } from "../wechat/vendor/weixin/run-lock.js";
import {
	getCapabilityConfig,
	applyAiEnvAnswers,
	writeEnvFile,
	reloadAiEnv,
	envPath,
} from "../wechat/vendor/weixin/ai-config.js";
import { resolveStateDir } from "../wechat/vendor/weixin/storage/state-dir.js";
import {
	createLoginSession,
	beginLogin,
	watchLogin,
...
/** Resolve account locally (no dependency on upstream driver.js). */
		...
		if (tools !== undefined && tools !== null && typeof tools.register === "function") {
			try {
				const disposeTool = tools.register({
					...
					async execute(args) {
						const { generateImage } = await import("../wechat/vendor/weixin/ai-service.js");
