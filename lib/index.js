 ...
 * Shape: { features: {<key>: bool}, featureConfig: {<key>: {...}} } —
 * the latter holds per-feature settings (方案A: merged over the module's
 * optional `defaultConfig` export). Older config files without
 * featureConfig are handled (empty object), so existing toggles survive.
 */
function readConfig() {
	let stored = null;
	try {
	...
	const featureConfig = {};
	for (const feature of FEATURES) {
		const base = feature.defaultConfig !== undefined && feature.defaultConfig !== null
			? feature.defaultConfig
			: {};
		const storedCfg = stored && stored.featureConfig && typeof stored.featureConfig === "object"
			? stored.featureConfig[feature.key]
			: undefined;
		featureConfig[feature.key] = (storedCfg !== undefined && storedCfg !== null && typeof storedCfg === "object")
			? { ...base, ...storedCfg }
			: { ...base };
	}
...
/** Client-facing snapshot: feature metadata + enabled flags + per-feature config (no live objects). */
function snapshot(config) {
	return {
		features: FEATURES.map((feature) => ({
			...
			// 该功能是否有配置项（featureConfig 非空时设置页渲染表单/卡片）。
			hasConfig: feature.defaultConfig !== undefined,
		})),
...
export function apply(ctx) {
	...
	const knownMethods = new Set(["config", "config/set", "config/feature", "ping", ...featureMethods.keys()]);
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			try {
				...
				if (suffix === "config/feature") {
					// 方案A：写某功能的配置项（featureConfig），合并其模块默认值；
					// 配置变化不触发 host 重注册（客户端经 store 实时响应）。
					const key = String(payload && payload.key ? payload.key : "");
					const feature = FEATURES.find((entry) => entry.key === key);
					if (feature === undefined) throw new Error(`未知功能 ${JSON.stringify(key)}`);
					const next = payload && payload.config !== undefined && payload.config !== null && typeof payload.config === "object"
						? payload.config
						: {};
					const base = feature.defaultConfig !== undefined && feature.defaultConfig !== null
						? feature.defaultConfig
						: {};
					config.featureConfig[key] = { ...base, ...next };
					saveConfig(config);
					writeOk(res, snapshot(config));
					return;
				}
