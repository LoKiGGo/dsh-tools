 ...
 *
 * 参考 dsh-weixin-gateway 的 driver.js 改写（MIT 来源标注见 README）：
 * 保留长轮询收消息、AgentBridge 回复、流式发送/媒体降级，新增 sender
 * allowlist 门禁，并去掉 CLI/appExit 依赖。停止通过 AbortController 控制。
 */

import { getUpdates, notifyStart, sendTyping } from "./vendor/weixin/api/api.js";
import { MessageItemType } from "./vendor/weixin/api/types.js";
import { WeixinConfigManager } from "./vendor/weixin/api/config-cache.js";
import { sendMessageWeixin } from "./vendor/weixin/send.js";
import { sendWeixinMediaFile } from "./vendor/weixin/send-media.js";
import { WeixinStreamingSender } from "./vendor/weixin/streaming-sender.js";
import { weixinMessageToMsgContext } from "./vendor/weixin/inbound.js";
import { downloadMediaFromItem } from "./vendor/weixin/media/media-download.js";
import { saveWechatMedia } from "./media-store.js";
import { askAgentStreaming } from "./vendor/bridge.js";
import { SessionRouter } from "./vendor/weixin/session-router.js";
import { logger } from "./vendor/weixin/util/logger.js";
import { getCapabilityConfig } from "./vendor/weixin/ai-config.js";
...
/** Pure decision: should the inbound image prompt hint at modlens_read_image? */
export function shouldUseModlensFallback(visionConfigured, toolNames) {
	if (visionConfigured === true) return false;
	if (!Array.isArray(toolNames)) return false;
	return toolNames.some((name) => typeof name === "string" && name.includes("modlens"));
}

/** True when the modlens vision tool is registered in the current DSH tools registry. */
export function hasModlensTool(ctx) {
	const tools = ctx?.get ? ctx.get("tools") : undefined;
	if (tools === undefined || tools === null) return false;
	try {
		if (typeof tools.get === "function" && tools.get("modlens_read_image") !== undefined) return true;
		const view = typeof tools.view === "function" ? tools.view() : undefined;
		if (view !== undefined && view !== null && view.visible instanceof Map) {
			for (const name of view.visible.keys()) {
				if (typeof name === "string" && name.includes("modlens")) return true;
			}
		}
	} catch {}
	return false;
...
async function handleIncoming(ctx, agentHandle, account, msg, baseUrl, token, configManager) {
	...
	if (mediaOpts.decryptedVoicePath) {
		...
		} else if (hasModlensTool(ctx)) {
			prompt = `[收到一张图片，已保存到: ${mediaOpts.decryptedPicPath}。可用 modlens_read_image 工具读取该图片] ${text}`;
		} else {
			prompt = `[收到一张图片，已保存到: ${mediaOpts.decryptedPicPath}] ${text}`;
		}
...
async function transcribeVoice(filePath) {
	try {
		const { transcribeAudio } = await import("./vendor/weixin/ai-service.js");
...
async function describeInboundImage(filePath) {
	try {
		const { describeImage } = await import("./vendor/weixin/ai-service.js");
